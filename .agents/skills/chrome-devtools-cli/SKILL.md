...
This machine uses Microsoft Edge. Every explicit daemon start must use:

```powershell
Start-Process (Get-Command chrome-devtools -ErrorAction Stop).Source -ArgumentList @(
    "start",
...
Never invoke chrome-devtools start directly. Direct invocation keeps foreground process attached and blocks agent from continuing.
Never start without both `--executablePath`,  `--no-performance-crux` and `--no-usage-statistics`.

## AI Workflow

1. **Initialize once**: On first browser task in an agent session, run `chrome-devtools status`. If daemon is not running, use Edge start command from Setup. Do not restart running daemon.
2. **Execute**: Run tools directly (e.g., `chrome-devtools list_pages`). Do not run `start`/`status`/`stop` before every command.
3. **Inspect**: Use `take_snapshot` to get an element `<uid>`.
4. **Act**: Use `click`, `fill`, etc. State persists across commands.
...
Usage statistics must remain disabled. `--no-usage-statistics` belongs on every explicit `start` command, not tool commands.
...
```bash
chrome-devtools start --executablePath "C:\Program Files (x86)\Microsoft\Edge\Application\msedge.exe" --no-usage-statistics # Start or restart with Edge and telemetry disabled
